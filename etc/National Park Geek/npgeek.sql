...
select * from park order by parkName ASC

select parkCode, parkName
from park

select parkCode,
Count(parkCode) as stuff
from survey_result
group by parkCode
...
select survey_result.parkCode, park.parkName, Count(survey_result.parkCode) as stuff from survey_result inner join park ON survey_result.parkCode = park.parkCode group by survey_result.parkCode, park.parkName order by stuff DESC
...
SELECT weather.parkCode, weather.fiveDayForecastValue, weather.forecast, weather.high, weather.low, park.parkName FROM weather Left join park ON weather.parkCode = park.parkCode where weather.parkCode = 'ENP'